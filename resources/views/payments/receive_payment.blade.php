            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                   @if(Session::has('flash_message'))
                    ...
                    {!! Form::open(['method'=>'POST','action'=>['PaymentController@receive_payment',$charge_invoice->charge_invoice_number]]) !!}
                    ...
                    <div class="row spacing">
                        <table class="table table-bordered table-striped" id="tableSortable" style="text-transform:uppercase;">
                            ...
                            <tbody>
                                <?php $x=0; ?>
                                @foreach($cust->invoice as $data)
                                    <tr id="total{{$x}}">
                                        <td>
                                            @if($data->charge_invoice_number == $charge_invoice->charge_invoice_number)
                                                <input type="checkbox" name="id[]" value="{{ $data->id }}" class="checks" checked/>
                                            @else
                                                <input type="checkbox" name="id[]" value="{{ $data->id }}" class="checks"/>
                                            @endif
                                        </td>
                                        <td>{{ $data->dt }}</td>
                                        <td>
                                        ...
                                        <td>
                                            @if($data->payment != NULL)
                                                P {{ number_format($data->invoice->sum('amount') - $data->payments->sum('payment'),2) }}

                                                {!! Form::hidden('payments[]',$data->invoice->sum('amount') - $data->payment->sum('payment'),['class'=>'form-control','class'=>'collectible']) !!}
                                            @else
                                                P {{ number_format($data->invoice->sum('amount'),2) }}
                                        ...
                                        <td>
                                            @if($data->charge_invoice_number == $charge_invoice->charge_invoice_number)
                                                <input type="text" name="payment_made[<?php echo $data->id; ?>][py]" value="{{$data->invoice->sum('amount') - $data->payments->sum('payment')}}" class="form-control payment">
