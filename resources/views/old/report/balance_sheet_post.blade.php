  ...
  <div class="col-lg-12">
    <h2 style="color: #2F4050; font-size: 16px; font-weight: 400; margin-top: 18px">
    	<ol class="breadcrumb">
			...
			<li class="breadcrumb-item">Balance Sheet As of <strong>{{Carbon::parse($all['to'])->toFormattedDateString()}}</strong></li>
		</ol>
    </h2>
  </div>
</div>
            ...
            <div class="panel panel-default">
                ...
                {!! Form::open(['action'=>'ReportController@balance_sheet_post','class'=>'form-inline'])!!}
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="idate" name="to" datepicker="" data-date-format="yyyy-mm-dd" data-auto-close="true" value="{{$all['to']}}">
					</div>
					                    ...
					                    <tr>
						                    ...
						                    @foreach($acc_type->coa as $coa)
						                    	<?php 
										          ...
										          $item_coa = App\coaItem::where('chart_of_account_id',$coa->id)
										              ->whereBetween('created_at',['1990-02-02',$all['to']])
										              ->get();
						                    	?>
						                    	@unless(!$item_coa->first())
						                    	<?php
						                    		$total += $item_coa->sum('debit')-$item_coa->sum('credit');
						                    	?>
						                    	<tr >
							                        <td>{{$coa->account_title}}</td>
							                        <td>{{number_format($item_coa->sum('debit')-$item_coa->sum('credit'),2)}}</td>
							                    </tr>
							                    @endunless
							                    ...
							                    @unless(!$coa->subaccount)
							                    	...
							                    	<?php 
											          ...
											          $item_coa_sub = App\coaItem::where('chart_of_account_id',$sub->id)
											              ->whereBetween('created_at',['1990-02-02',$all['to']])
											              ->get();
							                    	?>

							                    		@unless(!$item_coa_sub->first())
							                    		<?php 
							                    			$total += $item_coa_sub->sum('debit')-$item_coa_sub->sum('credit');

						                    			?>
								                    	<tr >
									                        <td style="text-indent: 10px;">&rarr; {{$sub->account_title}}</td>
									                        <td>{{number_format($item_coa_sub->sum('debit')-$item_coa_sub->sum('credit'),2)}}</td>
									                    </tr>
									                    @endunless
							                    	@endforeach
							                    @endunless
							                    ...
							                    <?php 
							                    	$recItem = App\recurringItem::where('chart_of_account_id',$coa->id)
							                    		->whereBetween('date',['1990-20-02',$all['to']])->get();
							                    ?>
