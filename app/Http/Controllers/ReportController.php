...
class ReportController extends Controller
{
    ...
    public function inventory_report(){
        ...
        function item($datas,$parent = 0){
            ...
            if($datas[$parent])
            {
                ...
                $asset_val_total = 0;
                    ...
                    $total_asset += $asset_val_total;
                    $total_onhand += $onhandparent;

                    session::put('global_total_asset', $totalasset+=$total_asset);
                    session::put('global_total_onhand', $totalonhand+=$total_onhand);
                        if($data->sub){ 
                            if(count($data->sub) > 0){
                                
                                 $child = item($datas, $data->id);
                    ...
                    if(count($data->sub) > 0){
                        $html .= '<tr>
                          ...
                          <td></td>
                          <td></td>
                          <td class="text-right" >'.number_format($total_onhand,2).'</td>
                          <td></td>
                          <td></td>
                         ...
                         $html .= '<tr>
                          ...
                          <td></td>
                          <td></td>
                          <td class="text-right" >'.number_format($onhandparent,2).'</td>
                          <td></td>
                          <td></td>
                          <td class="text-right">'.number_format($asset_val_total,2).'</td>
                        </tr>';
                    }
        ...
        $inventories = Item::with([
                ...
                'sub.item_flow.ci_item',
                'sub.item_flow.ci',
                'sub.item_flow.ci.customer_belong',

                'sub.item_flow.dr_item',
                'sub.item_flow.dr',
                'sub.item_flow.inventory_item',
                'sub.item_flow.inventory',

                'sub.item_flow.invoice',
                'sub.item_flow.invoice_item',
                'sub.item_flow.invoice.customer_belong',
